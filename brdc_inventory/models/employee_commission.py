...
class employee_commission(models.Model):
    _name = 'employee.commission'
    _rec_name = 'so_id'
    _description = 'List of Employee Commission'

    # name = fields.Char()
    so_id = fields.Many2one('sale.order', string="Ref No", readonly=True)
    # currency_id = fields.Many2one('res.currency', related='so_id.currency_id')
    currency_id = fields.Many2one('res.currency', default = lambda self: self.env.user.company_id.currency_id)

    sales_agent_id = fields.Many2one('res.partner', string="Sales Agent", readonly=True)
    unit_manager_id = fields.Many2one('res.partner', string="Unit Manager", related="sales_agent_id.unit_manager_id", readonly=True)
    agency_manager_id = fields.Many2one('res.partner', string="Agency Manager", related="sales_agent_id.agent_manager_id", readonly=True)
    ...
    agent_commission_line = fields.One2many('agent.commission.line','employee_commission_id')
    ...
    def compute_commission(self):
        ...
        if self.contract_price < self.selling_price:
            self.sa_comm = ((self.contract_price * .20) * (self.sales_agent_id.agency_id.comm_percent / 100))
            self.um_comm = ((self.contract_price * .20) * (self.unit_manager_id.agency_id.comm_percent / 100))
            self.am_comm = ((self.contract_price * .20) * (self.agency_manager_id.agency_id.comm_percent / 100))
        elif self.selling_price < self.contract_price:
            self.sa_comm = ((self.selling_price * .20) * (self.sales_agent_id.agency_id.comm_percent / 100))
            self.um_comm = ((self.selling_price * .20) * (self.unit_manager_id.agency_id.comm_percent / 100))
            self.am_comm = ((self.selling_price * .20) * (self.agency_manager_id.agency_id.comm_percent / 100))
        else:
            self.sa_comm = ((self.selling_price * .20) * (self.sales_agent_id.agency_id.comm_percent / 100))
            self.um_comm = ((self.selling_price * .20) * (self.unit_manager_id.agency_id.comm_percent / 100))
            self.am_comm = ((self.selling_price * .20) * (self.agency_manager_id.agency_id.comm_percent / 100))
        ...
        # return super(employee_commission, self).create(values)
    ...
    def divide_commissions(self):
        ...
        if self.env['sale.order'].search([('id', '=', self.so_id.id)]).purchase_term == 'cash':
            ...
            if self.env['sale.order'].search([('id', '=', self.so_id.id)]).new_payment_term_id.name == '12 mos':
        ...
        for x in range(0,div_range):
            ...
            agent_line_id = self.env['agent.commission.line'].create({
                'employee_commission_id': self.id,
